...
def compute_mse_loss(real, protected_scan, start_idx, end_idx, length, action="inject", save_img_result=True):
    if start_idx > 32 and (start_idx + 32) < length:
        try:
            ...
            target = torch.tensor(netManipulator.scan[start_idx:end_idx]).unsqueeze(1).float().to(device)
            target = (target - target.min()) / (target.max() - target.min())
            target = 2 * target - 1
            if save_img_result:
                all_imgs = torch.cat((real, protected_scan, target),0)
                vutils.save_image(vutils.make_grid(all_imgs, padding=5, normalize=True), "./images/mse_"+str(iters)+".png") 
...
#GENERATOR
netG = Generator(1,1,512).float().to(device)
netG.apply(weights_init)
print(netG)

#DISCRIMINATOR
netD = Discriminator(ngpu).float().to(device)
